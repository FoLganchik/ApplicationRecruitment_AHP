package com.example.applicationrecruitment.graph;
...
public class Graph {
    ...
    public static void draw() {
        if( overlay == null ){ overlay = MainApplication.rootLayoutController.overlay; }
        overlay.getChildren().clear();
        MainApplication.rootLayout.setCenter(overlay);
        nodes = getNodes();
    ...
    private static List<AHPNode[]> getNodes() {
        ...
        //HEAD
        PairwiseComparison head = PairwiseComparison.get(MainApplication.AHP.headId);
        nodes.add(new AHPNode[]{new AHPNode(head.id, null, head.alternatives)});

        //ALTERNATIVES
        AHPNode[] alternatives = new AHPNode[MainApplication.AHP.alternatives.size()];
        List<String> alternativeParents = AHP.getAlternativeParents();
        for( int i=0; i<alternatives.length; i++ ){
            alternatives[i] = new AHPNode(MainApplication.AHP.alternatives.get(i), alternativeParents, null);
        }
        nodes.add(alternatives);

        //CRITERIONS
        if( alternativeParents.contains(MainApplication.AHP.headId) ){
            return nodes;
        }
